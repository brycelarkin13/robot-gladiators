...
var fight = function(enemy) {
  ...
  while (playerInfo.health > 0 && enemy.health > 0) {
    if (fightOrSkip()) {
      // if true, leave fight by breakign loop
      break;
    }
    ...
    // switch turn order for the next order
...
for (var i = 0; i < enemyInfo.length; i++) {
  ...
  if (playerInfo.health > 0) {
    ...
    if (playerInfo.health > 0 && i < enemyInfo.length - 1) {
      // ask play if they want to use store before next round
...
var endGame = function() {
  ...
  if (playerInfo.health > 0) {
    window.alert("Great job, you've survived the game! You now have a score of a " + playerInfo.money + ".");
  }
  else {
